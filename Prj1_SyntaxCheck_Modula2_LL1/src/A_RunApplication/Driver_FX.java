...
public class Driver_FX extends Application {
	...
	public void start(Stage primaryStage) {
		try {
			...
			stage.getIcons().add(new Image(getClass().getResourceAsStream("/C_Graphic_icons/app_icon.png")));
			stage.show();
	...
	public BorderPane mainPage() {
		...
		Label l=new Label("");
		l.setPrefSize(450, 50);
		l.setStyle("-fx-background-color: white;-fx-Border-color: LIMEGREEN;-fx-Border-width:5;"
				+ "-fx-font-size:15;-fx-font-weight:bold;-fx-text-fill: LIMEGREEN");
		Button b_load=new Button("Load");
		b_load.setPrefSize(100, 25);
		b_load.setStyle("-fx-background-color: white; -fx-Border-color: black;"
    			+ "-fx-font-size:15;-fx-font-weight:bold; -fx-text-fill: 	black");
		hb_top.getChildren().addAll(l,b_load);
		hb_top.setPadding(new Insets(20,0,0,500)); // top, right, bottom, left
		hb_top.setSpacing(50);
		
		
		vb.getChildren().add(hb_top);
		vb.setPadding(new Insets(0,10,0,10)); // top, right, bottom, left
		// Center
		TextArea ta=new TextArea();
		ta.setStyle("-fx-background-color: white; -fx-Border-color: black;"
    			+ "-fx-font-size:15;-fx-font-weight:bold; -fx-text-fill: black");
		...
		b_load.setOnAction(e->{
        	...
        	try {
        		...
        		al_code_string.clear();
        		al_code_no_filter.clear();
        		hm_code_line.clear();
        		...
        		res="";
             	...
             	// read data thats inside the file.
             	read_modula2_code_file(selectedFile);
             	ta.setText(code);
             	// check code
             	check_code_syntax();
		...
		while(!stack.isEmpty() || !al_code_string.isEmpty()) {
			...
			if(al_code_string.get(0)==null) {
				...
				if(stack.peek().equals(";")) {
					...
					res="Line ("+num_line_error+"): "+hm_code_line.get(num_line_error)+" the error in missing "+string_error;
				}
				else {
					...
					res="Line ("+num_line_error+"): "+hm_code_line.get(num_line_error)+" the error in "+string_error;
				}
				break;
			}
			else {
				...
				if(stack.peek().equals(al_code_string.get(0))) {
					...
					if(production_number.equals("") || production_number.equals(" ")) {
						...
						if(stack.peek().equals(";")) {
							...
							res="Line ("+(num_line_error-1)+"): "+hm_code_line.get(num_line_error-1)+" the error in "+string_error;
						}
						else {
							string_error=al_code_no_filter.get(al_code_no_filter.size()-al_code_string.size());
							System.out.println("Line ("+num_line_error+"): "+hm_code_line.get(num_line_error)+" the error in "+string_error);
